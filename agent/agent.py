...
def worker():
    while True:
        ...
        if action == "create_room":
            ...
            # Lấy roomCode gần nhất của userId
            ...
            # Tạo bản ghi mới với roomCode cũ và rentalTime mới
            payload = {"userId": userId, "roomCode": room_code, "rentalTime": rentalTime, "status": "active"}
            print(f"[DEBUG] POST body gửi lên BE: {payload}")
...
def run_action(action, room_name=None):
    """
    Hỗ trợ các action (mới, theo cửa sổ):
      - click_window: {window, rel_x, rel_y}
      - click_image_in_window: {window, image, threshold?, region_ratio?}
      - ocr_copy_window: {window, region_ratio}
      - click_min_number_window: {window, region_ratio}
      - scroll_window: {window, amount}  (amount âm = cuộn xuống)
      - wait: {seconds}

    Vẫn hỗ trợ tạm thời (cũ, tuyệt đối):
      - click (image|x,y), ocr_copy(region px), click_min_number(region px),
        click_bottom_icon(image, region px), close_app(image),
        scroll_up_max, scroll_down_max, type, type_room_name, copy_clipboard
    """
    ...
    if action_type == "click_window":
        ...
        print(f"[INFO] click_window '{window}' at ratio ({rel_x},{rel_y}) -> ({abs_x},{abs_y})")
        return None

    if action_type == "click_image_in_window":
        window = action.get("window", "LDPlayer")
        image_path = action.get("image")
        threshold = float(action.get("threshold", 0.75))
        region_ratio = action.get("region_ratio")  # [x,y,w,h] 0..1
        rect = window_rect_abs(window)
        if rect is None:
            return None
    ...
    if action_type == "ocr_copy_window":
        window = action.get("window", "LDPlayer")
    ...
    if action_type == "click_min_number_window":
        window = action.get("window", "LDPlayer")
        region_ratio = action.get("region_ratio")
        rect = window_rect_abs(window)
        if rect is None:
    ...
    if action_type == "scroll_window":
        window = action.get("window", "LDPlayer")
        amount = int(action.get("amount", -1200))  # âm = xuống
        rect = window_rect_abs(window)
        if rect is None:
            return None
    ...
    # --------------- CŨ: tương thích tuyệt đối (nên chuyển dần) ---------------
    if action_type == "click":
        ...
        if image_path:
            ...
            if center:
                ...
                print(f"[WARN] Không tìm thấy {image_path} bằng OpenCV (best_score={score:.2f})")
        else:
    ...
    if action_type == "click_min_number":
        region = action.get("region")
        if not region:
    ...
    if action_type == "click_bottom_icon":
        image_path = action.get("image")
        region = action.get("region")
        center, score = find_image_opencv(image_path, region=tuple(region) if region else None,
                                          threshold=float(action.get("threshold", 0.75)))
        if center:
            pyautogui.click(center)
            print(f"[INFO] Click bottom icon {image_path} tại {center} (score={score:.2f})")
        else:
            print(f"[WARN] Không tìm thấy icon {image_path} (best_score={score:.2f})")
        return None
    ...
    if action_type == "scroll_down_max":
        pyautogui.scroll(-9999999)
        print("[INFO] Scrolling down to max...")
        return None

    if action_type == "scroll_up_max":
        pyautogui.scroll(9999999)
...
def get_latest_roomcode(userId):
    # Gọi BE để lấy bản ghi gần nhất của userId
    res = requests.get(f"{BACKEND_API}?userId={userId}&_sort=createdAt&_order=desc&_limit=1")
    if res.status_code == 200 and res.json():
        latest = res.json()[0]
        return latest.get("roomCode")
    return None
...
def run_extend_tool(roomCode):
    """
    Hàm giả lập extend room.
    Thêm 1 màn hình LDPlay vào roomCode đã có.
    """
    print(f"[TOOL] Extend room {roomCode} → thêm 1 LDPlay")
    # TODO: ở đây bạn code thao tác tool thật sự
    return True
